...
are interpolated with os.environ before the're set:
    ...
    ...
    Develop: '/sample-buildout/recipes'
    Keeping editable install of /sample-buildout/recipes: its packaging metadata (setup.py, setup.cfg, pyproject.toml) is unchanged since the previous run
    Reusing editable install: /sample-buildout/develop-eggs/recipes.egg-link
    ...
    Uninstalling extdemo.
    ...
    Installing extdemo.
